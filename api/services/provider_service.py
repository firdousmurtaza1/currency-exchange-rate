...
class ProviderService:
    ...
    def get_exchange_rate_data(source_currency, exchanged_currency, valuation_date) -> Decimal:
        ...
        for provider in providers:
            try:
                ...
                exchange_rate = provider.get_exchange_rate(source_currency, exchanged_currency, valuation_date)
    ...
    def get_exchange_rate_timeseries(self, source_currency, exchanged_currency, date_from, date_to):
        ...
        for provider in Provider.objects.filter(is_active=True).order_by("priority"):
            ...
            try:
                rates_data = adapter.fetch_exchange_rate_timeseries(source_currency, exchanged_currency, date_from, date_to)
    ...
    def get_conversion_rate(self, source_currency, exchanged_currency, amount):
        ...
        for provider in Provider.objects.filter(is_active=True).order_by("priority"):
            adapter = self.PROVIDERS[provider.name]
            try:
                rate_data = adapter.fetch_conversion_rate(source_currency, exchanged_currency, amount)
                if rate_data:
                    return rate_data
            except Exception:
                continue
...
def get_exchange_rate_data(source_currency, exchanged_currency, valuation_date, provider: ExchangeRateProvider) -> Decimal:
