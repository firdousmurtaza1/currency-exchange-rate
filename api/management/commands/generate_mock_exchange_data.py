...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            base_currency_obj, created = Currency.objects.get_or_create(
                code=base_currency
            )
            ...
            while current_date <= end_date:
                for symbol in symbols:
                    ...
                    exchanged_currency, created = Currency.objects.get_or_create(
                        code=symbol
                    )

                    # Generate a random exchange rate within a realistic range (1 to 150 for example)
                    rate_value = Decimal(random.uniform(1.0, 150.0)).quantize(
                        Decimal("0.000001")
                    )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully generated mock exchange rates from {start_date_str} to {end_date_str}."
                )
            )
            ...
            self.stdout.write(
                self.style.ERROR(
                    f"File 'data.json' not found in the current directory."
                )
            )
        except json.JSONDecodeError:
            self.stdout.write(
                self.style.ERROR(f"Error decoding JSON in file 'data.json'.")
            )
